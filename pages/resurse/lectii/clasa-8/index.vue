<template>
  <div class="p-8">
    <h1 class="text-3xl font-bold mb-6 text-sky-950">Materii pentru Clasa 8</h1>
    <div class="grid grid-cols-2 sm:grid-cols-3 gap-6">
      <SelectionCard
        v-for="subject in subjects"
        :key="subject"
        :to="`/resurse/lectii/clasa-8/${subject}`"
      >
...
onMounted(async () => {
  const files = await import.meta.glob(`~/server/data/lessons/clasa-8/*.json`)
